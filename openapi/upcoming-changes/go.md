...
* Add support for `BilliePayments` on `AccountCapabilitiesParams` and `AccountCapabilities`
...
* Add support for new value `billie` on enums `ConfirmationTokenPaymentMethodPreview.Type` and `PaymentMethod.Type`
...
* Add support for new value `billie` on enum `PaymentLink.PaymentMethodTypes`
